...
class FullMetaDatasetH5(torch.utils.data.Dataset):
        ...
        for i, dataset_name in enumerate(datasets):
            ...
            for class_id in class_set:
                ...
                self.class_images[dataset_name][class_id] = [
                    str(j) for j in range(dataset_spec.get_total_images_per_class(class_id))
                ]
                # breakpoint()
    ...
    def __getitem__(self, idx):
        ...
        for class_id, nb_support, nb_query in episode_description:
            assert nb_support + nb_query <= len(
...
class ContrastiveMetaH5(torch.utils.data.Dataset):
    def __init__(self, args, split=Split["TRAIN"]):
        super().__init__()
        ...
        self.datasets = datasets
        # Contrastive learning. Resize only.
        self.transforms = get_transforms(data_config, Split.VALID)
        self.len = episod_config.num_episodes * len(
            datasets
        )  # NOTE: not all datasets get equal number of episodes per epoch
    ...
    def __getitem__(self, idx):
        ...
        return support_images, support_labels, None, None
