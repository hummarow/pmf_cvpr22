...
def _check_validity_of_restricted_classes_per_split(
    restricted_classes_per_split: Dict[Split, int], classes_per_split: Dict[Split, int]
):
    ...
    for split_enum, num_classes in restricted_classes_per_split.items():
        if split_enum not in [Split.TRAIN, Split.VALID, Split.TEST]:
            raise ValueError(
                "Invalid key {} in restricted_classes_per_split."
        ...
        if num_classes > classes_per_split[split_enum]:
            raise ValueError(
                "restricted_classes_per_split can not specify a "
                ...
                "only {} available for that split.".format(
                    num_classes, split_enum, classes_per_split[split_enum]
                )
            )
...
def get_total_images_per_class(data_spec, class_id: int = None):
    ...
    if class_id is None:
        if len(set(data_spec.images_per_class.values())) != 1:
            raise ValueError(
                "Not specifying class_id is okay only when all classes"
    ...
    if class_id not in data_spec.images_per_class:
        raise RuntimeError(
            "The DatasetSpecification should be regenerated, as "
...
class BenchmarkSpecification(
    ...
    def __new__(
        ...
        if len(has_dag_ontology) != len(dataset_spec_list):
            raise ValueError("The length of has_dag_ontology must be the number of " "datasets.")
        if len(has_bilevel_ontology) != len(dataset_spec_list):
            raise ValueError(
                "The length of has_bilevel_ontology must be the number " "of datasets."
            )
        if len(splits_to_contribute) != len(dataset_spec_list):
            raise ValueError(
                "The length of splits_to_contribute must be the number " "of datasets."
            )
        ...
        for i, has_dag in enumerate(has_dag_ontology):
            if has_dag and not isinstance(dataset_spec_list[i], HierarchicalDatasetSpecification):
                raise ValueError(
                    "Dataset {} has dag ontology, but does not have a "
            ...
            if not has_dag and isinstance(dataset_spec_list[i], HierarchicalDatasetSpecification):
                raise ValueError(
                    "Dataset {} has no dag ontology, but is represented "
        ...
        for i, is_bilevel in enumerate(has_bilevel_ontology):
            if is_bilevel and not isinstance(dataset_spec_list[i], BiLevelDatasetSpecification):
                raise ValueError(
                    "Dataset {} has bilevel ontology, but does not have a "
        ...
        for dataset_spec, dataset_splits in zip(dataset_spec_list, splits_to_contribute):
            ...
            if invalid_train_split or invalid_valid_split or invalid_test_split:
                raise ValueError(
                    "A dataset can not contribute to a split if it has "
...
class DatasetSpecification(
    collections.namedtuple(
        ...
        ["name", "classes_per_split", "images_per_class", "class_names", "path", "file_pattern"],
    )
):
    ...
    def initialize(self, restricted_classes_per_split: Dict[Split, int] = None):
        ...
        if self.file_pattern not in ["{}.tfrecords", "{}_{}.tfrecords"]:
            raise ValueError(
                'file_pattern must be either "{}.tfrecords" or '
    ...
    def get_total_images_per_class(self, class_id: int = None):
    ...
    def to_dict(self, ret_Dict):
        ...
        ret_Dict["classes_per_split"] = {
            split.name: count for split, count in six.iteritems(ret_Dict["classes_per_split"])
        }
...
class BiLevelDatasetSpecification(
    ...
    def initialize(self, restricted_classes_per_split: Union[Split, int] = None):
        """Initializes a DatasetSpecification.
    ...
    def _get_split_offset(self, split: Split):
        ...
        if split == Split.TRAIN:
            ...
            previous_superclasses = range(0, self.superclasses_per_split[Split.TRAIN])
            offset = self._count_classes_in_superclasses(previous_superclasses)
        elif split == Split.TEST:
            previous_superclasses = range(
                0,
                self.superclasses_per_split[Split.TRAIN] + self.superclasses_per_split[Split.VALID],
            )
            offset = self._count_classes_in_superclasses(previous_superclasses)
        else:
            raise ValueError("Invalid dataset split.")
    ...
    def get_classes(self, split: Split):
        ...
        if (
            self.restricted_classes_per_split is not None
            and split in self.restricted_classes_per_split
        ):
    ...
    def get_class_ids_from_superclass_subclass_inds(
        self, split: Split, superclass_id: int, class_inds: List[int]
    ):
    ...
    def to_dict(self, ret_Dict):
        ...
        ret_Dict["superclasses_per_split"] = {
            split.name: count for split, count in six.iteritems(ret_Dict["superclasses_per_split"])
        }
...
class HierarchicalDatasetSpecification(
    collections.namedtuple(
        ...
        ["name", "split_subgraphs", "images_per_class", "class_names", "path", "file_pattern"],
    )
):
    ...
    def initialize(self, restricted_classes_per_split: Dict[Split, int] = None):
        ...
        self.class_names_to_ids = dict(zip(self.class_names.values(), self.class_names.keys()))
    ...
    def get_all_classes_same_example_count(self):
        ...
        # `set1 | set2` is union
        is_class_balanced = (
            len(train_example_counts) == 1
            and len(valid_example_counts) == 1
            and len(test_example_counts) == 1
            and len(train_example_counts | valid_example_counts | test_example_counts) == 1
        )
    ...
    def get_total_images_per_class(self, class_id=None, pool=None):
        ...
        if pool is not None:
            raise ValueError(
                "No dataset with a HierarchicalDataSpecification "
        ...
        if class_id is None:
            if common_num_class_images < 0:
                raise ValueError(
                    "class_id can only be None in the case where all "
    ...
    def to_dict(self, ret_Dict):
        ...
        for split, synset_counts in six.iteritems(ret_Dict["images_per_class"]):
            wn_id_counts = {synset.wn_id: count for synset, count in six.iteritems(synset_counts)}
...
def as_dataset_spec(dct: Dict[str, Any]):
    ...
    def _key_to_split(dct):
        ...
        return {Split[key]: value for key, value in six.iteritems(dct)}

    # Early returns:
    if "__class__" not in dct:
        return dct
    if dct["__class__"] not in [
        "DatasetSpecification",
        "HierarchicalDatasetSpecification",
        "BiLevelDatasetSpecification",
    ]:
    ...
    if dct["__class__"] == "DatasetSpecification":
        ...
        for class_id, n_images in six.iteritems(dct["images_per_class"]):
            ...
            if isinstance(n_images, dict):
                ...
                n_images = {pool: int(pool_size) for pool, pool_size in six.iteritems(n_images)}
            else:
        ...
        for split in Split:
            ...
            for synset in split_subgraphs[split]:
                ...
                if wn_id in wn_id_to_node:
                    raise ValueError("Multiple `Synset` objects associated to the same WordNet ID")
                wn_id_to_node[wn_id] = synset

        images_per_class = {}
        for split_name, wn_id_counts in six.iteritems(dct["images_per_class"]):
            synset_counts = {
                wn_id_to_node[wn_id]: int(count) for wn_id, count in six.iteritems(wn_id_counts)
            }
